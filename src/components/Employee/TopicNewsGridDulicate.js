...
export default function TopicNewsGrid() {
  ...
  const [isStreaming, setIsStreaming] = useState(false); // NEW: tracks server streaming state
  const limit = 9; // or whatever number of articles per fetch
  ...
  // keep your existing fetchArticles function (still used if you want server-side REST fallback)
  const fetchArticles = async (pageToFetch = 1) => {
    try {
      if (pageToFetch === 1) {
        setLoading(true); // initial load or refresh
      } else {
        setLoadingMore(true); // loading more pages
      }
  ...
  // Setup WS connection and streaming behavior
 const setupWebSocket = (topic, region, page = 1, limit = 9) => {
  ...
  // If this is a fresh first-page subscription, reset visible articles
  if (page === 1) {
    setArticles([]); // reset UI so skeletons represent the full first-page slots
  }

  // keep requestedCount for skeleton calculation (used by rendering)
  setRequestedCount(limit);

  ws.onopen = () => {
    console.log("WebSocket connected");
    // indicate streaming starting
    setIsStreaming(true);

    // set loading flags according to page
    if (page === 1) {
      setLoading(true);
      setLoadingMore(false);
    } else {
    ...
    // subscribe message (backend expects this structure)
  ...
  ws.onmessage = (event) => {
    try {
      ...
      // Accept either a single article or a bulk array from the server
      if (message.type === "new-article") {
        const incoming = message.article ? [message.article] : (message.articles || []);
        ...
        setArticles((prev) => {
          ...
          for (const art of incoming) {
            ...
            if (!next.find((a) => a.title === art.title)) {
              next.push(art);
            }
          }

          // If this is the first page, make sure we never exceed the requested limit
          if (page === 1 && next.length > limit) {
        ...
        // server done streaming for this subscription
        setIsStreaming(false);
        setLoading(false);
        setLoadingMore(false);

        // optionally decide hasMore based on server info in message
  ...
  ws.onclose = () => {
    ...
    wsRef.current = null;
    // If WS closes unexpectedly while streaming, stop skeletons
    setIsStreaming(false);
    setLoading(false);
    setLoadingMore(false);
  };
  ...
  // loadArticles triggers WS streaming for the requested page (page 1 or load more)
  const loadArticles = (pageToLoad = 1) => {
    if (!selectedTopic || !selectedRegion) return;

    // set flags that will be overridden by ws.onopen when it starts
    if (pageToLoad === 1) {
      setLoading(true);
    } else {
      setLoadingMore(true);
    ...
    // Safety fallback: if stream doesn't end after X seconds, clear loading flags
    // (but prefer to rely on stream-end from backend)
    ...
    // clear fallback when streaming ends or wsRef changes
    const cleanupWatcher = () => {
      clearTimeout(fallback);
    };

    // small safe cleanup: when socket becomes available, attach a close watcher
    // Not perfect but avoids memory leaks
    return cleanupWatcher;
  };

  // When topic or region changes, reset and load first page via streaming
useEffect(() => {
  setArticles([]);
  setPage(1);
  setHasMore(true);
  setIsStreaming(true); // ✅ important: start streaming state immediately
  loadArticles(1);
  // eslint-disable-next-line react-hooks/exhaustive-deps
}, [selectedTopic, selectedRegion]);


  // INITIAL LOADS: topics and region detection
  useEffect(() => {
  ...
  // When user changes topic or region manually, reset page and articles (UI side)
  useEffect(() => {
  ...
  // Compute skeleton count: how many more cards should be shown while streaming
  const totalSlots = requestedCount; // e.g. 9 per page
  const remainingSlots = Math.max(totalSlots - articles.length, 0);
...
{isStreaming &&
  Array.from({ length: limit - articles.length }).map((_, index) => (
    <Card
      key={"skeleton-" + index}
      sx={{
        width: isMobile ? "100%" : "calc(33.33% - 16px)",
      ...
      <CardContent>
        ...
        <Box mt={4} sx={{ display: "flex", justifyContent: "center", width: "100%" }}>
          <Button
            ...
            onClick={() => {
              // trigger next page streaming while keeping existing articles visible
              const next = page + 1;
              setPage(next);
              loadArticles(next);
            }}
